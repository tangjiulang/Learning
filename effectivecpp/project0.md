...
//
//  Copyright (p1) 2015-2022, Carnegie Mellon University Database Group
//
// ===----------------------------------------------------------------------===//
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    while (p1 != key.end()) {
      ...
      if (!pre_chi->get()->HasChild(*cur)) {
        pre_chi = pre_chi->get()->InsertChildNode(*cur, std::make_unique<TrieNode>(*cur));
      } else {
        pre_chi = pre_chi->get()->GetChildNode(*cur);
      }
    }
    p1--;
    auto end_node = pre_chi->get()->GetChildNode(*p1);
    ...
    pre_chi = pre_chi->get()->InsertChildNode(*p1, std::make_unique<TrieNode>(*p1));
    auto new_node = new TrieNodeWithValue(std::move(**pre_chi), value);
    pre_chi->reset(new_node);
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    while (p1 != key.end()) {
      auto cur = p1++;
      if (pre_chi->get()->HasChild(*cur)) {
        s.push(std::make_tuple(*cur, pre_chi));
        pre_chi = pre_chi->get()->GetChildNode(*cur);
        continue;
      }
      latch_.WUnlock();
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    while (p1 != key.end()) {
      auto cur = p1++;
      auto next_node = pre_chi->get()->GetChildNode(*cur);

      if (!next_node) {
        *success = false;
        break;
      }
      if (next_node->get()->IsEndNode() && p1 == key.end()) {
...
$this$ 可以访问类中所有的数据成员，包括 $public,protect,private$

#### 左值和右值的概念

左值是 $lvalue$ 不是 $left\ value$ 的简写，而是 $loactor\ value$ 指在内存中，有明确存储地址的（可寻址的数据）

右值是 $rvalue$ 不是 $right\ value$ 的简写，而是 $read\ value$ 指可以提供数据值的数据
...
#### $std::fowrad$ 和 $std::move$

$std::move$ 将对象强行改成右值。

$std::foward$ 实现了参数在传递过程中保持其值属性的功能，即若是左值，则传递之后仍然是左值，若是右值，则传递之后仍然是右值。