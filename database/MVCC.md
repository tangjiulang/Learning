...
MVCC的目的就是多版本并发控制，在数据库中的实现，就是为了解决读写冲突，它的实现原理主要是依赖记录中的隐式字段，undo日志 ，Read View 来实现的。
...
| name    | age  | DB_ROW_ID（隐式主键） | DB_TRX_ID（事务 ID) | DB_ROLL_PTR（回滚指针） |
| ------- | ---- | --------------------- | ------------------- | ----------------------- |
...
* min_trx_id：read view 生成时，活跃事务 id 列表中的最小 id
